...
const Canvas = ({ onPixelChange, width, height, pixels }) => {
    ...
    fetch("/api/canvas")
      ...
      .then((res) => {
        for (let pixel of res) {
          //console.log(pixel)
          pixels.map((el) =>
            el.x == pixel.x * 10 && el.y == pixel.y * 10
              ? (el.color = `#${pixel.colour.toString(16)}`)
              : el
          );
  ...
  const handleClickPixel = (event) => {
    ...
    if (clickedPixel) {
      const translationX =
        -clickedPixel.x + canvas.width / 2 - window.innerWidth / 2;
      const translationY =
        -clickedPixel.y + canvas.height / 2 - window.innerHeight / 2;
